...
| Code scanning | CodeQL analyses JavaScript/TypeScript; PHP is intentionally unsupported by CodeQL | CodeQL runs source-only TypeScript analysis with least-privilege permissions. PHP remains covered by PHPStan, configuration/schema checks, tests and review; this assurance baseline is not a PHP SAST claim. No external PHP SAST provider is enabled. | Repository maintainer; triage every alert, review coverage when GitHub adds PHP support or an approved SAST decision exists. |

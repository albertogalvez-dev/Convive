...
- **Every Action referenced in `.github/workflows/ci.yaml` is pinned to an immutable commit SHA**, not a mutable version tag — a tag like `@v6` can be moved by its publisher to point at different code without any change appearing in this repository's history; a SHA cannot. Each pin carries a trailing `# vX.Y.Z` comment purely for human readability; Dependabot's `github-actions` ecosystem update recognises this convention and bumps both the SHA and the comment together when a new release ships.

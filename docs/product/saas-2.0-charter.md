...
| #509 | T-17, C-9 (context) | INV-16 | environment purposes (local/test/sandbox/demo-prod/pilot); release/flag/config/rollback; no fictional↔customer co-mingling — decided in **ADR-0032** | — |
...
| 1.2 | 2026-09-05 | **#509** decided as **ADR-0032**: five environment purposes (`local`, `test`, `demo-production`, `saas-sandbox`, `saas-pilot` — the last named now but not yet stood up, reserved for #538's go decision), a single `APP_ENVIRONMENT_PURPOSE` config value as INV-16's concrete separation mechanism, config-gated feature flags (never a runtime toggle) so incomplete SaaS surfaces can merge to `main` under real CI, and a separate release/rollback action reusing the demo's runbook shape against `saas-sandbox` instead of the demo's own deployment. |

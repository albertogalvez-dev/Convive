...
| [ADR-0012](0012-use-cloudflare-tunnel-for-the-single-vps-deployment.md)          | Use Cloudflare Tunnel for the single-VPS deployment                     | Public ingress, TLS, production trust boundaries, image delivery, secrets, migration release and rollback policy                                    | Accepted |
| [ADR-0013](0013-use-restic-with-off-host-object-storage-for-database-recovery.md) | Use Restic with off-host object storage for database recovery           | Encrypted database snapshots, failure-domain separation, retention, restore verification and credential invalidation                                | Accepted |

...
2. a short-lived, report-scoped capability grant represented by an opaque
   protected cookie after an anonymous reporter proves possession of the
   report's access secret.
...
- normalise Unicode input to NFC before hashing and measure password length in
  Unicode code points rather than storage bytes;
...
and copies it to a configurable header on mutating same-origin requests.

The first walking-skeleton security spike must therefore select, document and
verify the supported token lifecycle separately for:

- login, logout and professional-session mutations;
- anonymous capability mutations, which do not have a Symfony session.

The spike may adapt the frontend to Symfony's stateless protocol or use an
appropriate backend-issued token through maintained Symfony facilities. This is
an explicit implementation decision inside the architectural constraints below,
not permission to invent an unreviewed custom token algorithm merely to connect
two framework defaults. No cookie-authenticated state-changing feature is
complete until its lifecycle passes the integration tests.

Every selected lifecycle must ensure that:
...
The development proxy selected in ADR-0005 preserves the same relative
`/api/v1` browser contract locally, so development does not introduce a second
credentialed browser origin.
...
To enter the private follow-up area, the reporter submits only the access secret
in a request body. The public tracking reference remains a non-secret receipt
and support identifier and does not participate in authentication. Neither the
secret nor the later capability handle is placed in a URL.

The access secret uses the 256-bit, 64-character lowercase hexadecimal profile
selected in ADR-0010. The later capability handle will contain at least 128 bits
of entropy generated by a cryptographically secure random source; its exact
length and unambiguous encoding will be fixed during implementation without
reducing that minimum. Input length will be bounded before lookup or
verification work, and collisions must fail safely.

The backend will:

1. apply rate limits before expensive verification;
2. derive the deterministic one-way representation of the submitted secret;
3. locate the report through that unique indexed representation without
   revealing whether it exists, is revoked or is otherwise unavailable;
4. create a new, short-lived capability grant scoped only to the permitted
   follow-up operations for that report;
5. return the same safe failure representation for every invalid, unknown or
   revoked secret.
...
A restoration runbook must keep authenticated and capability-based access
closed until it has also purged sessions, invitation and reset tokens and has
reconciled account disablement, security revisions, memberships, revoked report
secrets, deletion state and audit continuity. The deployment decision will
select the concrete recovery and global invalidation mechanism and test it
against the required recovery objectives.
...
report requires its access secret again. This prevents one cookie from
accumulating grants for several reports.
...
Anonymous access-secret verification returns a generic failure that does not
disclose whether the secret is unknown, revoked or linked to an unavailable
report.
...
- anonymous access-secret verification without enumeration;
...
- the exact access-secret and capability-handle lengths and encodings above the
  selected 128-bit minimum;
...
- Angular and Symfony have documented same-origin CSRF constraints and a
  mandatory integration-verification boundary.
...
- [PHP cryptographically secure random bytes](https://www.php.net/manual/en/function.random-bytes.php)

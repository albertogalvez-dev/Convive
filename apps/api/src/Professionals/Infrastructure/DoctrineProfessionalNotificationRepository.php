...
final readonly class DoctrineProfessionalNotificationRepository implements ProfessionalNotificationRepository
{
    ...
    public function findFor(Professional $professional, int $limit): array
    {
        return $this->entityManager
            ->getRepository(ProfessionalNotification::class)
            ->findBy(['recipient' => $professional], ['createdAt' => 'DESC', 'id' => 'DESC'], $limit);
    }
    ...
    public function findForRecipient(Uuid $id, Professional $professional): ?ProfessionalNotification
    {
        return $this->entityManager
            ->getRepository(ProfessionalNotification::class)
            ->findOneBy(['id' => $id, 'recipient' => $professional]);
    }
    ...
    public function enabled(Professional $professional, ProfessionalNotificationType $type): bool
    {
        ...
        return $this->preference($professional, $type)?->enabled() ?? true;
    }

    public function changePreference(Professional $professional, ProfessionalNotificationType $type, bool $enabled): void
    {
        if ($type->isRequired()) {
            throw new LogicException('A safeguarding-required notification type cannot be disabled.');
        }

        $preference = $this->preference($professional, $type);
        if ($preference === null) {
            $this->entityManager->persist(new ProfessionalNotificationPreference($professional, $type, $enabled));
        } else {
            $preference->change($enabled);
        }

        $this->entityManager->flush();
    }

    private function preference(Professional $professional, ProfessionalNotificationType $type): ?ProfessionalNotificationPreference
    {
        return $this->entityManager
            ->getRepository(ProfessionalNotificationPreference::class)
            ->findOneBy(['professional' => $professional, 'type' => $type]);
    }
}

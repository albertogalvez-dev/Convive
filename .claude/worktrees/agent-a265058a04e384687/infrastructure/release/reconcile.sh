...
readonly RUNTIME_DIR=${CONVIVE_RUNTIME_DIR:-/srv/convive}
readonly SECRET_DIR=${CONVIVE_SECRET_DIR:-/etc/convive/secrets}
...
for required_file in \
    "${COMPOSE_FILE}" \
    "${SECRET_DIR}/cloudflare_tunnel_token" \
    "${SECRET_DIR}/api.env" \
...
for secret_file in "${SECRET_DIR}/cloudflare_tunnel_token" "${SECRET_DIR}/api.env" "${SECRET_DIR}/database_password" "${SECRET_DIR}/redis.conf"; do
...
compose up --detach --remove-orphans api gateway cloudflared
